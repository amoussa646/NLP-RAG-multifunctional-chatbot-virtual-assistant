...
def determine_information_type(query):
    prompt = f"Determine if the following query requires 'factual information' or 'personal information': '{query}', return the answer only , just the classification"
    response = client.chat.completions.create(model="gpt-4-turbo",
     messages=[
                {"role": "system", "content": systemPrompt},
                {"role": "user", "content": prompt}
...
def handle_factual_information(query):
    response = client.chat.completions.create(model="gpt-4-turbo",
     messages=[
                {"role": "system", "content": systemPrompt},
                {"role": "user", "content": query}
...
def handle_personal_information(query):
    # Assuming RAG_Personal is defined and available
    return RAG_Personal(query)

def extract_intent_and_entities(query):
    prompt = f"Example query: 'move forward 50 meteres', the output should be JSON like this: 'intent : move, entities: [direction : forward , distance : 50] ,  Extract the intent and entities from the following task execution query: '{query}'"
    response = client.chat.completions.create(model="gpt-4-turbo",
     messages=[
                {"role": "system", "content": systemPrompt},
                {"role": "user", "content": prompt}
...
def handle_query(query):
    ...
    response = 'Error'
    if query_type == "chatting":
        print("chatting")
        response = handle_chatting(query)
    elif query_type == "information":
        info_type = determine_information_type(query)
        print(info_type)
        if info_type == "factual information":
            print("factual information")
            response = handle_factual_information(query)
        elif info_type == "personal information" or  info_type == "Personal information":
            print("personal information")
            response = handle_personal_information(query)
    elif query_type == "task execution":
        print("task execution ")
        intent_and_entities = extract_intent_and_entities(query)
        response = f"Intent and Entities: {intent_and_entities}"

    return response


def chat():
    print("AI: Hi there! Let's chat. You can type 'exit' to end the conversation.")
    while True:
...
# # Example usage
# query = "i am bored tell me a joke "
